...
class CategorySerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if len(data['title']) < 3:
            raise serializers.ValidationError('Category title lengh should be at least 3')
        return data
...
class OrderCreateSerializer(serializers.Serializer):
    ...
    def validate_cart_id(self, cart_id):
        if not Cart.objects.filter(id=cart_id).exists():
            raise serializers.ValidationError('there is no cart with this cart')
        
        if CartItem.objects.filter(cart_id=cart_id).count() == 0:
            raise serializers.ValidationError('your cart is empty. please add some product.')
